...
(function () {
    ...
    */
    class DrawCurve{//拉伸曲线
        ...
        //贝塞尔曲线实现//
        fineBezier(points,num) {//返回贝塞尔曲线各点数据数组
            var $this = this;
   ...
   //****************************高度测量 第一个点的经纬度，第二个点的高度，两点水平距离为半径************************************************//
    var measureHeight = function (viewer){		
        ...
        handler.setInputAction(function(movement){
            ...
            LineMovement(viewer,[
                { longitude: point.x, dimension: point.y, height: point2cartographic.height, time: 5 },
                { longitude: point.x, dimension: point.y, height: point.z, time: 0 }
            ]);

            VMSDS.core.QueryModel_Entities_Name(viewer,'测量直线').forEach(element => {
                viewer.entities.remove(element);
            });
            // VMSDS.core.QueryModel_Entities_Name(viewer,'初始高度').forEach(element => {
            //     viewer.entities.remove(element);
            // });
    ...
    const version = "1.0.0"
    let M = { expando: "measure" + ( version + Math.random() ).replace( /\D/g, "" )}
