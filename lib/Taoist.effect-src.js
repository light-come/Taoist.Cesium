...
(function () {
        ...
        switch (type) {
            ...
            case '着色器1':
                ...
                */
                ...
                Cesium.Material.PolylineTrailLinkImage = Cesium.buildModuleUrl(
                ...
                Cesium.Material.Source = "czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                {\n\
                    czm_material material = czm_getDefaultMaterial(materialInput);\n\
                    vec2 st = materialInput.st;\n\
                    vec4 colorImage = texture2D(image, vec2(fract(-(st.t + time)), st.t));\n\
    ...
    function  DynamicLine(viewer,list) {
        ...
        */
        ...
        Cesium.PolylineTrailLinkMaterialProperty = PolylineTrailLinkMaterialProperty;
        Cesium.Material.PolylineTrailLinkType = 'PolylineTrailLink';
        Cesium.Material.PolylineTrailLinkImage =  Cesium.buildModuleUrl("../../core/images/line.png");
        Cesium.Material.PolylineTrailLinkSource = "czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                                                    {\n\
                                                        czm_material material = czm_getDefaultMaterial(materialInput);\n\
                                                        vec2 st = materialInput.st;\n\
    ...
    const version = "1.0.0"
    let E = { expando: "effect" + ( version + Math.random() ).replace( /\D/g, "" )}
    ...
    effect.forEach(element => {
      E[element.getName()] =element
    });
    G.extend({
      E
    });
    if ( typeof noGlobal === "undefined" ) {
      window.effect = window.E = E;
    }
