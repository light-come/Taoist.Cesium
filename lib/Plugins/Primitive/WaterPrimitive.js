...
class WaterPrimitive {
  ...
  constructor(options) {
    ...
    // 初始化Uniform变量
    this._initUniforms = {
      normalMapUrl: Cesium.defaultValue(options.normalMapUrl, "/assets/images/waterNormals.jpg"),
  ...
  _createReflectionWaterMaterial() {
    ...
    const { normalMapUrl, size, waterColor, waterAlpha, rf0, lightDirection, sunShiny, distortionScale } = this._initUniforms;
  ...
  _updateVirtualCamera(camera) {
    ...
    // 视图坐标下的水面
    const clipPlane = new Cesium.Cartesian4(reflectorPlane.normal.x, reflectorPlane.normal.y, reflectorPlane.normal.z, reflectorPlane.distance);

    // 拷贝当前反射相机的投影矩阵
    const projectionMatrix = Cesium.Matrix4.clone(this._virtualCamera.frustum.projectionMatrix);

    const q = new Cesium.Cartesian4((Math.sign(clipPlane.x) + projectionMatrix[8]) / projectionMatrix[0], (Math.sign(clipPlane.y) + projectionMatrix[9]) / projectionMatrix[5], -1, (1.0 + projectionMatrix[10]) / projectionMatrix[14]);
  ...
  _getFixedFrameToEastNorthUpTransformFromWorldMatrix() {
    ...
    const fixedFrameToEastNorthUpTransform = Cesium.Matrix4.inverse(eastNorthUpToFixedFrameTransform, new Cesium.Matrix4());
