...
(function (global, factory) {
  ...
  Taoist.dMod_X = function (viewer, _model) {
    ...
    var _arr_entitys = [];
    for (var i = 0; i < entitys.length; i++) {
      if (!_model)   viewer.entities.remove(entitys[i]);else
    ...
    for (var i = 0; i < viewer.scene.primitives.length; i++) {
      if (!_model)  viewer.scene.primitives.remove(viewer.scene.primitives.get(i));else
   ...
   * @param options
   */
  Taoist.aTiles = function (viewer, options) {
    var tileset;
      ...
      .then(function () {
        ...
        if (options.flyTo) {
          if(!Taoist.aTiles.Timeout)
            Taoist.aTiles.Timeout =  setTimeout(() => {//开始飞行
   ...
   */
   Taoist.IA = Taoist.InterceptionAndmonitoring = (e,a) => {
    ...
    function indexOf(_url) {
      if (_url.indexOf('bin') != -1 
      || _url.indexOf('gltf') != -1 
      || _url.indexOf('glb') != -1 
      || _url.indexOf('tileset.json') != -1
      || _url.indexOf('b3dm') != -1  
      || _url.indexOf('cmpt') != -1	){
        return true
      }else{
