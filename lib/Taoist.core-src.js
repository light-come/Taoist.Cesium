...
(function (global, factory) {
   ...
   */
  Taoist.aGLTF = async function (viewer, options) {
    var model;
    ...
    model = viewer.scene.primitives.add(gltf);
   ...
   * @param uri 高程资源
   */
  Taoist._TP = async function (viewer, uri) {
    let provider = await Cesium.CesiumTerrainProvider.fromUrl(uri ?? "http://121.40.42.254:8008/%E6%9D%AD%E5%B7%9E-%E9%AB%98%E7%A8%8B/data/", {
   ...
   * @param options
   */
  Taoist.aTiles = async function (viewer, options, e) {
    var tileset;

    var tileset = await Cesium.Cesium3DTileset.fromUrl(options.url, options);
