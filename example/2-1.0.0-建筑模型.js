...
window.onload = () =>{
  const _ = (_viewer,params) => {
    if(_viewer instanceof  Cesium.Viewer)
      new Gear(_viewer)[params]()
  }
  /**
   * 添加模型
   * @param {*} url 模型链接
   */
  function a3DTiles(flyTo,url) {
    //添加模型
    G.a3DTiles(viewer,{
      id: _(viewer,"uuid"),//生成随机id_
  ...
  (function () {
    ...
    function _build(response,a3DTiles) {
      $("#mapBox").append(`<div class="tree well infoview" style="max-height: 666px;overflow: auto;-ms-overflow-style: none;scrollbar-width: none;">
          <ul>
              <li id="tree_ul">
                  <span><i class="icon-folder-open"></i> `+'建筑'+`</span> 
              </li>
          </ul>
      </div>`)
      var data = response
      ...
      function makeTree(parentObj, treeJson) {
          ...
          for (var i = 0; i < treeJson.length; i++) {
              ...
              if(treeJson[i].uri != null){
                  a3DTiles( (i == 0 ? false : true),treeJson[i].uri )
